...
public class PerInterfaceMetric {
    ...
    public static void main(String[] args) throws IOException {
        ...
        BufferedWriter writer = Files.newBufferedWriter(Paths.get("../tcc-outputs/cost/per-project/AIO-per-interface-metrics.csv"));
        CSVPrinter csvPrinter = new CSVPrinter(writer, CSVFormat.DEFAULT
                .withHeader("interfaces", "metric").withDelimiter(';'));
        ...
        map.forEach((k, v) -> {
            ...
            try {
                ...
                csvPrinter.printRecord(k.replace(':', '-'), new BigDecimal(avarage).setScale(2, RoundingMode.HALF_DOWN).doubleValue());
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
